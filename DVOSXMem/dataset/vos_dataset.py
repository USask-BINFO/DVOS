...
class VOSDataset(Dataset):
    ...
    """
    # Define a constant training size of 256. The original size was 384. 
    TRAIN_SIZE = 384
    def __init__(self, im_root, gt_root, max_jump, is_bl, subset=None, num_frames=3, max_num_obj=3, finetune=False):
        ...
        if self.is_bl:
            ...
            self.all_im_dual_transform = transforms.Compose([
                ...
                transforms.RandomResizedCrop((self.TRAIN_SIZE, self.TRAIN_SIZE), scale=(0.25, 1.00), interpolation=InterpolationMode.BILINEAR)
            ])

            self.all_gt_dual_transform = transforms.Compose([
                transforms.RandomHorizontalFlip(),
                transforms.RandomResizedCrop((self.TRAIN_SIZE, self.TRAIN_SIZE), scale=(0.25, 1.00), interpolation=InterpolationMode.NEAREST)
            ])
        else:
            self.all_im_dual_transform = transforms.Compose([
                transforms.RandomHorizontalFlip(),
                transforms.RandomResizedCrop((self.TRAIN_SIZE, self.TRAIN_SIZE), scale=(0.36,1.00), interpolation=InterpolationMode.BILINEAR)
            ])

            self.all_gt_dual_transform = transforms.Compose([
                transforms.RandomHorizontalFlip(),
                transforms.RandomResizedCrop((self.TRAIN_SIZE, self.TRAIN_SIZE), scale=(0.36,1.00), interpolation=InterpolationMode.NEAREST)
            ])
    ...
    def __getitem__(self, idx):
        ...
        while trials < 5:
            ...
            for f_idx in frames_idx:
                ...
                png_name = frames[f_idx].replace("_frame-", "_mask-")
        ...
        cls_gt = np.zeros((self.num_frames, self.TRAIN_SIZE, self.TRAIN_SIZE), dtype=np.int64)
        first_frame_gt = np.zeros((1, self.max_num_obj, self.TRAIN_SIZE, self.TRAIN_SIZE), dtype=np.int64)
