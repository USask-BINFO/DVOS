...
for si, stage in enumerate(stages_to_perform):
    ...
    def renew_vos_loader(max_skip, finetune=False):
        train_dataset = VOSDataset(
            path.join(wheat_root, "frames"), 
            path.join(wheat_root, "masks"), 
            max_skip, is_bl=False,
    ...
    if stage == '0':
        ...
        wheat_root = path.join(path.expanduser(config["wheat_root"]), "training")
